...
module.exports = async function (req,res) {
    try {
        ...
        if(!(title&&thumbnail&&synopsis&&author&&content&&category&&tag)
            ...
            if(!title){
                return res.json({code:0, msg: "文章标题不能为空", datas: []})
            }else if(!thumbnail){
                return res.json({code:0, msg: "文章缩略图不能为空", datas: []})
            }else if(!synopsis){
                return res.json({code:0, msg: "文章简介不能为空", datas: []})
            }else if(!author){
                return res.json({code:0, msg: "文章作者不能为空", datas: []})
            }else if(!content){
                return res.json({code:0, msg: "文章内容不能为空", datas: []})
            }else if(!category){
                return res.json({code:0, msg: "文章分类不能为空", datas: []})
            }else if(!Array.isArray(tag)){
                return res.json({code:0, msg: "文章标签必须为数组", datas: []})
            }else if(thumbnail.indexOf("/uploads/")!==0){//验证图片路径格式
                return res.json({code:0, msg: "缩略图路径格式不符(/uploads/xxx)", datas: []})
            }else if(title.length<2||title.length>50){
                return res.json({code:0, msg: "文章标题长度必须在2-50之间", datas: []})
            }else if(synopsis.length<2||synopsis.length>200){
                return res.json({code:0, msg: "文章简介长度必须在2-200之间", datas: []})
            }else if(!isObjectId(author)){
                return res.json({code:0, msg: "这不是一个合法的作者id", datas: []})
            }else if(!isObjectId(category)){
                return res.json({code:0, msg: "这不是一个合法的分类id", datas: []})
            }else if(tag.length>0 && !isObjectId(tag)){
                return res.json({code:0, msg: "这不是一个合法的标签id", datas: []})
            }else {
                return res.json({code:0, msg: "参数格式不满足", datas: []})
            }
        }else {
            //验证标题是否存在
            if(await articles.findOne({title})){
                return res.json({code:0, msg:"标题已经存在", datas: [],token:req.tokenObj.token})
            }
            //查证分类和标签是否存在
            ...
            if(tagDoc.length===tag.length && categoryDoc){//标签和分类有效
                ...
                await res.json({code:1, msg:"添加成功", datas: [newArticleDoc],token:req.tokenObj.token})
            }else{//标签或者分类无效
