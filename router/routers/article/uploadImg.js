...
module.exports = function(req,res){//文件上传配置
    ...
    upload.single('file')(req, res, function (err) {
        if(!req.file || req.file.size === 0){
            res.json({code:0,msg: "图片不能为空",  datas: [], token});
        }else if(req.file.mimetype!=="image/png"&&req.file.mimetype!=="image/jpeg"){
            res.json({code:0,msg: "只允许JPG、PNG文件上传",  datas: [], token});
        }else if(req.file.size > 1024*1024){
            res.json({code:0,msg: "只小于1MB的图片上传",  datas: [], token});
        } else {
            if (err) {//上传发生未知错误
                err_logs.addErrLog(req,err,__filename);
                res.json({code:500,msg: "上传失败",  datas: [] ,token});
            } else{
                res.json({code:1,msg: "上传成功",  datas: [{filepath: req.userUploadDir.slice(1) + req.file.filename}],token})//返回文件路径
            }
        }
    });
};
