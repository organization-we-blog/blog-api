...
ErrLogSchema.static('addErrLog', function (req, err_obj, err_file) {
  if (err_obj instanceof Error || (err_obj.name && err_obj.stack && err_obj.message)) {//检测err_obj是否是一个错误对象
    ...
    //将错误日志存入数据库
    this.create({ip, type, date, url, err_type, err_msg, req_data, err_file, err_status}, function (err) {
      if (err) {//错误日志存储出错（错上加错）
        ...
        fs.readFile("../../errorLog.json", function (error, data) {
          if (error) {//3三层错误，没救了
            ...
            console.log({ip, type, date, url, err_type, err_msg, req_data, err_file, err_status});//错误起源
            console.log(err);//数据库存储日志错误
            console.log(error);//文件读取日志错误
          } else {
            let datas = JSON.parse(data) || [];
            let dataErr = {
              start_err: {ip, type, date, url, err_type, err_msg, req_data, err_file, err_status},//原始错误信息
              bd_err: err//数据库错误信息
            };
...
let ErrLog = mongoose.model('err_logs', ErrLogSchema);
